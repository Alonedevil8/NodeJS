...
const admin = async (req, res) => {
  ...
  try {
    ...
    const [propiedades, total] = await Promise.all([
      // Busca todas las propiedades del usuario y las incluye con información de categoría y precio.
      Propiedad.findAll({
        limit,
        offset,
        where: {
...
const crear = async (req, res) => {
  // Obtiene todas las categorías y precios disponibles.
  const [categorias, precios] = await Promise.all([
    Categoria.findAll(),
    Precio.findAll(),
  ]);
  // Renderiza la vista de creación de propiedad con datos de categorías y precios.
...
const guardar = async (req, res) => {
  ...
  await check("wc")
    .isNumeric()
    .withMessage("Selecciona el Numero de Baños")
    .run(req);
  ...
  if (!resultado.isEmpty()) {
    // Si resultado viene con datos, obtiene categorías y precios y muestra la vista de creación con errores.
    const [categorias, precios] = await Promise.all([
      Categoria.findAll(),
      Precio.findAll(),
    ]);
...
const editar = async (req, res) => {
  ...
  // Renderiza la vista de edición con datos de la propiedad y categorías y precios.
...
const guardarEdicion = async (req, res) => {
  ...
  if (!resultado.isEmpty()) {
    // Si hay errores, obtiene categorías y precios y muestra la vista de edición con errores.
    const [categorias, precios] = await Promise.all([
      Categoria.findAll(),
      Precio.findAll(),
    ]);
